		...
		//DEMONSTRATE FLOODING
			...
			if(!use_api && !single_kernel_flood_example)
			{
				...
				if(flood_from_borders)
				{
					...
					{
						scamp7_kernel_begin();
							PROP_1(RZ);//Propagate 1s from both Flooding Source and boundaries of the Array
						scamp7_kernel_end();
					}
				}
				else
				{
					...
					{
						scamp7_kernel_begin();
							PROP_0(RZ);//Propagate 1s from both Flooding Source
						scamp7_kernel_end();
					}
				}

				scamp7_kernel_begin();
					MOV(S1,RZ);//Copy the result of Flooding into S1
